...
```
**Note:** The build may take several hours as per your system performance. Provide good cooling for your host sytem.

After successfull build, we'll have an image that we can flash on a micro SD card.

In case of any errors, [Yocto Dev Manual](https://docs.yoctoproject.org/dev-manual/) is an awsome place to learn about Debugging Tools and Techniques.
...
Once it is done, insert the SD card in to raspberry pi5 and power ON raspberry pi5 after connecting HDMI monitor and keyboard.
...
Navigate to build folder and initialize the build enviorment.
...
Also verify the network connectivity after connect ethernet, use `ip -a` to verify the IP assignment.
...
Navigate to build folder and initialize the build enviornment.
```bash
source oe-init-build-env
```
...
bitbake core-image-base
```
Flash the image in to raspberry pi 5.
...
execute `hello` from the raspberry pi 5 and verify that our application has been sucessfully included in the image.
```bash
hello
```
It will be givig an output as:
```bash
Rohin Just tested a 'Hello, World!' :-)

```

Yess!! Our `Hello World` app is indcluded in the image:-)
